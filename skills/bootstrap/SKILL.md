...
8. **設計成果物の役割分担を AGENTS.md に埋め込む**:
   - 要件 / 思想・規約 / 設計 / 計画 / 実装の各成果物が持つ canonical 領域と、「他成果物の情報は転記せず参照する」原則
   - 実装計画のスコープ (何を・どの順で・どこまで のみ) と、方法論・規約は AGENTS.md 側に置くという境界
   - スライス / タスク / PR の 3 階層が粒度ではなく目的で分かれること
   - `resources/context-template.md` の表をプロジェクトの技術スタックに合わせて具体化する (該当しない行は削除)

**AGENTS.md (原典) に含める内容** (resources/context-template.md 参照):
- プロジェクトの概要 (Phase 1 の要約)
- 開発の原則 (philosophy/ から選定)
- 品質基準 (perspectives/ から選定)
- 設計成果物の役割分担 (canonical 領域、実装計画のスコープ、3 階層の目的)
