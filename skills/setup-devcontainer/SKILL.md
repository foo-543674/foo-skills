...
- `extensions`: linter, formatter, 言語サポート、テストランナー（詳細は「2.4 VS Code 拡張の選定基準」参照）
...
- [ ] macOS / Windows（WSL2）両方で動作する設定になっているか（改行コード、パス、ボリュームマウント）
- [ ] features の提供元が信頼できるソースか（`ghcr.io/devcontainers/*` 優先）
- [ ] VS Code 拡張の発行元が認証済み（チェックマーク付き）か

## ローカルインフラとの連携

**原則: devcontainer とローカルインフラ（DB・キャッシュ等）は疎結合にする。**

devcontainer.json の `dockerComposeFile` でローカルインフラの `docker-compose.yml` を直接参照する構成は避ける。この構成では devcontainer を起動するたびに DB 等が一緒に起動し、不要なリソース消費・起動時間の増大を招く。

**推奨構成: ネットワーク共有による疎結合**

1. devcontainer は単独で起動する（`image` または専用の `docker-compose.yml`）
2. ローカルインフラは setup-local-infra スキルで別途管理する独立した `docker-compose.yml` で起動する
3. 両者は Docker ネットワークを共有し、必要なときだけインフラを起動して接続する
...
// .devcontainer/devcontainer.json
{
  ...
  // devcontainer が参加する外部ネットワーク
  "runArgs": ["--network=my-app-network"],
  // ネットワークが未作成でもエラーにならないよう事前作成
  "initializeCommand": "docker network create my-app-network 2>/dev/null || true"
}
```

```yaml
# docker-compose.yml (ローカルインフラ側・setup-local-infra が管理)
services:
  postgres:
    image: postgres:16
    networks:
...
**このアプローチのメリット**:
- devcontainer の起動が速い（DB の起動を待たない）
- DB が不要な作業（フロントエンドのみ等）では DB を起動しなくてよい
- インフラの起動・停止を開発者が任意のタイミングで制御できる
...
- **OS 固有依存**: Windows 固有のパス記法やシェル構文に依存し、macOS 開発者が動かせない（逆も同様）
- **インフラ密結合**: `dockerComposeFile` で DB 等を devcontainer に同梱し、開発環境を起動するたびに全インフラが立ち上がる。起動が遅く、不要なリソースを消費する
